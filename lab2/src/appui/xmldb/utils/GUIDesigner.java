...
public class GUIDesigner {
    ...
    private static JMenu createMenu(Navigation nav) {
    ...
    private static JMenuItem createSubMenuItem(NavigationItem nav, ResourceBundle bundle) {
        ...
        menuItem.addActionListener(EventController.getListener(nav));
        return menuItem;
    }
    ...
    public static JToolBar initToolBar() {
        ...
        for (Navigation nav : Navigation.values()) {
            for (NavigationItem item : nav.getSubItems()) {
                ...
                button.addActionListener(EventController.getListener(item));
                toolBar.add(button);
            }
            toolBar.addSeparator();
        }
